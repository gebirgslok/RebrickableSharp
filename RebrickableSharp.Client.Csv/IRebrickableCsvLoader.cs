...
public interface IRebrickableCsvLoader : IDisposable
{
    ...
    Task<T[]> DownloadAsync<T>(CancellationToken cancellationToken = default) where T : class;
    ...
    Task<string?> DownloadFileAsync<T>(bool decompress = true, CancellationToken cancellationToken = default) where T : class;
    ...
    /// <returns>an array of <typeparamref name="T"/></returns>
    Task<T[]> ParseAsync<T>(string csvFileName, CancellationToken cancellationToken = default) where T : class;
}