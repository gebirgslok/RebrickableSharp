...
internal class JsonCsvClassMap<T> : ClassMap<T>
    where T : class
{
    /// <summary>
    ...
    public JsonCsvClassMap(IDictionary<string, string>? alternateNames = null)
    {
        ...
        foreach (var property in jsonProperties)
        {
            ...
            {
                var names = new List<string> { jsonPropertyName };
                if (
                    alternateNames != null
                    && alternateNames.TryGetValue(property.Name, out var alternateName)
                )
                {
